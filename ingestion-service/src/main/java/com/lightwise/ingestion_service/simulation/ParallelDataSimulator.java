    ...
    public void sendMockData() {
        ...
        for (int i = 0; i < parallelThreads; i++) {
            ...
            executorService.submit(() -> {
                DataSimulator(requestsForThread, random, restTemplate, ingestionEndpoint, log);
            });
        }
    }

    static void DataSimulator(int requestsForThread, Random random, RestTemplate restTemplate, String ingestionEndpoint, Logger log) {
        for (int j = 0; j < requestsForThread; j++) {
            ...
            try {
                ...
                restTemplate.postForEntity(ingestionEndpoint, dto, String.class);
                log.info("Sent mock data: {}", dto.toString());
            } catch (Exception e) {
